...
run_analysis <- function(data, cell_type, measurement_type) {
  ...
  # ---- Half-life with CI; free_y; NA upper CI shown as 100 ----------------
  make_hl_plot <- function(df, title, group_lookup = NULL) {
    # Cap undefined upper CI at 100 for display only
    df <- df %>%
      mutate(HalfLife_Upper_plot = ifelse(is.na(HalfLife_Upper), 100, HalfLife_Upper))
