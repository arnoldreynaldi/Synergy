...
plot_ratio <- function(numerator_label,
  ...
  ratio_data <- combined_data %>%
    ...
    pivot_wider(
      ...
      names_from  = Pop,
      values_from = Value,
      values_fn   = mean          # collapses duplicate rows safely
    )
  
  if (!all(c("NUM", "DEN") %in% names(ratio_data))) {
    warning("SKIP ", numerator_short, "/", denominator_short,
  ...
  p <- ggplot(
    ...
    labs(
      ...
      x     = "Day",
      y     = paste0(numerator_short, " / ", denominator_short, " ratio"),
      color = "Vaccine"
    ) +
    scale_x_continuous(
...
cat("\n===== CSVs produced =====\n")
print(list.files(out_dir, pattern = "_summary\\.csv$"))